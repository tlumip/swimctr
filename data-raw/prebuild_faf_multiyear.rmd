...
```{r first}
FN <- "~/Library/Mobile Documents/com~apple~CloudDocs/Data/FAF/FAF5.3.csv.xz"
fhwa_db <- swimctr:::load_tabular_data(FN)
...
internal_regions <- c(411, 419)   # Portland, rest of Oregon
```
...
The SWIM modeled area only includes Oregon and a halo of immediately adjacent counties. Thus, we need to associate FAF regions outside of the halo with the SWIM external stations they enter and exit the halo at. We used Google Maps to identify the shortest path between the largest city in each FAF region outside of Oregon and Salem. The latter is thought to represent the geographic center of the Portland-Eugene corridor, in which most economic activity is concentrated. The same process was used for flows through Oregon (e.g., from FAF regions in Washington to California). In this step we read these previously defined data. 
```{r four}
FN <- "~/Models/swimctr/data-raw/ct_external_regions.csv"
external_regions <- swimctr:::load_tabular_data(FN)
...
for (this_year in years_found) {
  process_this_year <- swimctr::preprocess_faf_database(append_distances,
    target_year = this_year, internal_regions = internal_regions,
